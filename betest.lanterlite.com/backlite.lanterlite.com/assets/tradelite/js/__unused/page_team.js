...
var URL = {}
URL['page'] = 'page_team/' + page_id + "/" + LANG
...
function page_set() {
	...
	var page_team = document.createElement('div')
	page_team.setAttribute('class', 'page_team')
	page_team.setAttribute('id', 'page_team')
	L.body.add(page_team)
	for(var i=0; i<TXT['page']['post'].length; i++) {
	  var keys = Object.keys(TXT['page']['post'][i]);
	  if (keys == 'title') {
	  	var txt = document.createElement('h1')
	  	txt.innerHTML = TXT['page']['post'][i][keys]
		  page_team.appendChild(txt)
	  }
	  else if (keys == 'sub') {
		  ...
		  page_team.appendChild(img)
	  }
	  else if (keys == 'img-body') {
	  	var div = document.createElement('div')
	  	$(div).css({"width": "100%", "text-align": "left"})
	  	var img = document.createElement('img')
	  	$(img).css({"width": "25%"})
	  	img.setAttribute("class", "std circle")
	  	img.src = BASE_URL + "assets/corelite/img/" + TXT['page']['post'][i][keys]
		  div.appendChild(img)
		  page_team.appendChild(div)
	  }
	  else if (keys == 'span-img') {
	  	var div = document.createElement('div')
	  	$(div).css({"width": "100%", "text-align": "center"})
	  	var span = document.createElement('span')
	  	span.innerHTML = TXT['page']['post'][i][keys]
		  ...
		  page_team.appendChild(a)
	  }
	  else if (keys == 'elm') {
	  	var elm = str_to_elm(TXT['page']['post'][i][keys])
		  page_team.appendChild(elm)
	  }
	  else if (keys == 'span') {
