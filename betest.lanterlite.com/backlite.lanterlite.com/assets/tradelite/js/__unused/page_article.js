...
var URL = {}
URL['page'] = 'page_article/' + article_id + "/" + LANG
...
function page_set() {
	...
	var page_article = document.createElement('div')
	page_article.setAttribute('class', 'page_article')
	page_article.setAttribute('id', 'page_article')
	L.body.add(page_article)
	for(var i=0; i<TXT['page']['post'].length; i++) {
	  var keys = Object.keys(TXT['page']['post'][i]);
	  if (keys == 'title') {
	  	var txt = document.createElement('h1')
	  	txt.innerHTML = TXT['page']['post'][i][keys]
		  page_article.appendChild(txt)
	  }
	  else if (keys == 'sub') {
	  ...
	  console.log('asd2')
		  ...
		  page_article.appendChild(a)
	  }
	  else if (keys == 'elm') {
	  	var elm = str_to_elm(TXT['page']['post'][i][keys])
		  page_article.appendChild(elm)
	  }
	  else if (keys == 'span') {
