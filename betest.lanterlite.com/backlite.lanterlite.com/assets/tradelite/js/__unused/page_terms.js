...
function page_set() {
	...
	div.setAttribute('style', ' color: black; line-height: 30px;')
	div.setAttribute('id', 'terms')

	var page_terms = document.createElement('div')
	page_terms.setAttribute('class', 'page_terms')
	page_terms.setAttribute('id', 'page_terms')
	L.body.add(page_terms)

	page_terms.appendChild(div)
	for(var i=0; i<TXT['page']['post'].length; i++) {
	  ...
	  if (keys == 'title') {
	  	...
	  	img.src = BASE_URL + "assets/corelite/img/page_terms/" + TXT['page']['post'][i][keys]
		  div.appendChild(img)
	  }
	  else if (keys == 'link') {
