...
function page_set() {
	...
	var page_compat = document.createElement('div')
	page_compat.setAttribute('class', 'page_compat')
	page_compat.setAttribute('id', 'page_compat')
	L.body.add(page_compat)
	for(var i=0; i<TXT['page']['post'].length; i++) {
	  var keys = Object.keys(TXT['page']['post'][i]);
	  if (keys == 'title') {
	  	var txt = document.createElement('h1')
	  	txt.innerHTML = TXT['page']['post'][i][keys]
		  page_compat.appendChild(txt)
	  }
	  else if (keys == 'sub') {
		  ...
		  page_compat.appendChild(a)
	  }
	  else if (keys == 'elm') {
	  	var elm = str_to_elm(TXT['page']['post'][i][keys])
		  page_compat.appendChild(elm)
	  }
	  else if (keys == 'span') {
