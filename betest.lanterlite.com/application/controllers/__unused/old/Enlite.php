...
class Enlite extends CI_Controller{
	...
	function searchFrom($db_file, $stringData) {
		...
		$stringData = $this->L->clean_string($stringData);
		$result_2 = $this->L->mysort3($stringData);
		...
		for ($i=0;$i<sizeof($result_2['words']); $i++) {
			...
			// $this->L->printJson($result_name);
		if ($result_name != null) {
			// $this->L->printJson($result_2);
	...
	function search2($stringData, $page) {
		...
		if ($type == "quran") {
			...
			// $this->L->printJson($asd_tot);
			// $this->L->printJson($asd_tot['correction']);
		}
	...
	function manageString($string, $word) {
		...
		if (sizeof($arr) != 0) {
			$index = 0;
			for ($i=0; $i<sizeof($arr); $i++) {
				if ($this->L->StrCmpCaseInsen($arr[$i], $word[0], 'part')) {
					$index = $i;
					$i = sizeof($arr);
				}
			}
			...
			for ($i=$index+1; $i<$index+20; $i++) {
				if (array_key_exists($i, $arr)) {
					...
					for ($j=0; $j<sizeof($word); $j++) {
						if ($this->L->StrCmpCaseInsen($arr[$i], $word[$j], 'part')) {
