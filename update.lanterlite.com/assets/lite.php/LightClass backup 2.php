...
class LightClass {
	...
	public function post_char_data($char_data) {
	  ...
	  if ($this->is_speak_key_pressed($char_data)) {
		  ...
		  if ($this->is_speak_key_pressed($char_data)) {
			...
			// 					$_status['char_health']['current'] -= 10;
	...
	// private function is_poke_key_pressed($char_data) {
	//   if ($this->L->arr_value_exist($char_data['pressed_keys'], $this->cross_key) && $this->L->arr_value_exist($char_data['pressed_keys'], $this->l1_key)) {
	//   	return true;
	//   }
	//   else {
   ...
   //    }
	...
	public function post_ask_to_join_guild($char_id, $asked_char_id, $guild_id) {
		$dir = BASE_DIR .'/storages/light/';
		$_json = $this->L->json_read($dir . 'players/'.$asked_char_id)[DATA];
		$_json['asked_to_join_guild']['guild_id'] = $guild_id;
		$_json['asked_to_join_guild']['asked_by'] = $char_id;
		$this->L->json_save($dir.'players/', $asked_char_id, $_json, $minify=false);
		return true;
	}

	public function post_confirm_yes_guild($char_id) {
		$dir = BASE_DIR .'/storages/light/';
		$player_main_data = $this->L->json_read($dir . 'players_status/'.$char_id)[DATA];
		$player_main_data['guild_id'] = $player_main_data['asked_to_join_guild']['guild_id'];
		$player_main_data['asked_to_join_guild'] = null;
