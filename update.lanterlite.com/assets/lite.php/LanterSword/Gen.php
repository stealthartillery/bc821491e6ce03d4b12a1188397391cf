...
// // 	$player_status = LGen('JsonMan')->read(dir.'players_status/'. $player_id);
// 	$increasable = true;
...
// 					LGen('JsonMan')->save(dir, 'players_status/'.$player_id, $player_status, $minify=false);
// 				}
// 			}
// 			sleep(2);
// 		}
...
function health_increase($player_id) {
	...
	$char_health = get_player($player_id, 'char_health');
	...
	if ($increasable and !get_player($player_id, 'is_health_increasing')) {
		...
		set_player($player_id, 'char_health', $char_health);
...
function post_char_data($char_data) {
	...
	$config = LGen('JsonMan')->read(BASE_DIR.'storages/sword/config');
	if ($config['propkey']) {
		$propkey = LGen('JsonMan')->read(BASE_DIR.'storages/sword/propkey');
