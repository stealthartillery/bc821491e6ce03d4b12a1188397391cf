<?php if (!defined('BASEPATH')) exit('no direct script access allowed');
	...
	class SwordGen2 {
		...
		// fixed
		public function get_char($char_id, $key='char_id') {
			$_obj = [];
			$_obj['namelist'] = [];
			...
			if ($key === 'health') {
				$_obj['def'] = 'chars/health';
				$_obj['namelist'] = ['current', 'max'];
				array_push($_obj['bridge'], LGen('StringMan')->to_json('{"id": "health", "puzzled":true}'));
			}
			else if ($key === 'exp') {
				$_obj['def'] = 'chars/exp';
				$_obj['namelist'] = ['current', 'max'];
				array_push($_obj['bridge'], LGen('StringMan')->to_json('{"id": "exp", "puzzled":true}'));
			}
			else if ($key === 'stamina') {
				$_obj['def'] = 'chars/stamina';
				$_obj['namelist'] = ['current', 'max'];
				array_push($_obj['bridge'], LGen('StringMan')->to_json('{"id": "stamina", "puzzled":true}'));
			}
			else if ($key === 'system_chats') {
				$_obj['def'] = 'chars/@system_chats';
				$_obj['namelist'] = ['id', 'chat'];
				array_push($_obj['bridge'], LGen('StringMan')->to_json('{"id": "system_chats", "puzzled":true}'));
			}
			else if ($key === 'appearence') {
