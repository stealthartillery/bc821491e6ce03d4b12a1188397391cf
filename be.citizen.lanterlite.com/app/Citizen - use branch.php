<?php
	...
	class CitizenGen {
		...
		public function get_ship_id($obj) {
			...
			$asd = LGen('SaviorMan')->get_all($obj);
			return $asd;
			$obj['bridge'] = array(
				0 => LGen('StringMan')->to_json('{"id":"ships","puzzled":true}'),
				1 => LGen('StringMan')->to_json('{"id":"'.$obj['val']['ship_id'].'","puzzled":true,"branch_no":'.$inc.'}')
			);
			$obj['namelist'] = ['client_ip', 'cit_id'];
			$obj['def'] = 'ships';
			...
			if (sizeof($result)>0) {
				...
				$_obj['def'] = 'citizens';
				$_obj['namelist'] = ['branch_no', 'created_date', 'verification_code', 'fsize', 'theme_color', 'theme_font', 'email', 'lang', 'is_verified', 'id', 'fullname', 'username', 'gender', 'address', 'phonenum', 'silver', 'point'];

				$citizen = LGen('SaviorMan')->read($_obj);
				$_result['id'] = $citizen['id'];
				$_result['branch_no'] = $citizen['branch_no'];
		...
		public function login($obj) {
			...
			// check citizens
			$branch = getFileNamesInsideDir($dir);
			if ($branch === LGen('GlobVar')->not_found)
				return LGen('GlobVar')->not_found;

			foreach ($branch as $bkey => $bval) {
				$citizens = getFileNamesInsideDir($dir.$bval.'/');
				...
				foreach ($citizens as $key => $value) {
					...
					if ($citizen['email'] === $email and $citizen['password'] === $password) {
						...
						$obj['namelist'] = ['branch_no', 'verification_code', 'created_date', 'fsize', 'theme_color', 'theme_font', 'email', 'lang', 'is_verified', 'id', 'fullname', 'username', 'gender', 'address', 'phonenum', 'silver', 'point'];
						$citizen = LGen('SaviorMan')->read($obj);
						// $result['ship_id'] = $ship_id;
						$result['id'] = $citizen['id'];
						$result['branch_no'] = $citizen['branch_no'];
