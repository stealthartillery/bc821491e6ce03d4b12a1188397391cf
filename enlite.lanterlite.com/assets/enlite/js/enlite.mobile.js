...
var URL = {}
URL['page'] = 'enlite/' + LANG
URL['header'] = 'enlite/component/header/' + LANG
URL['sidebar'] = 'enlite/component/sidebar/' + LANG
...
function text_get(count) {
 	json_read_local(HOME_URL + 'storage/languages/'+URL[KEYS[count]], function(obj) {
		TXT[KEYS[count]] = obj
		SET[count]()
		count += 1
...
function home_srcbar_create() {
	...
	$(title).css({
		'background': 'url(' + FE_URL + './assets/img/enlite/enlite.svg) no-repeat 5px 5px',
	  'background-size': '150px 50px',
	  'background-position': 'center'
	})

	var srcbar = searchbar_create()
  ...
  style += "box-sizing: border-box;"
	...
	function searchbar_create(on_submit) {
		var src_icon = document.createElement('img')
		src_icon.src = FE_URL + './assets/lite.img/src.svg'
...
function close_click() {
	// app["FSIZE_RSLIDER_AR"] = FSIZE_RSLIDER_AR,
	// app["FSIZE_RSLIDER_LAT"] = FSIZE_RSLIDER_LAT,
	// app["TOGGLE_TRANSLATION_CHECKED"] = TOGGLE_TRANSLATION_CHECKED,
	// app["TOGGLE_TRANSLITERATION_CHECKED"] = TOGGLE_TRANSLITERATION_CHECKED,
	// app["CHECKBOX_WBW_CHECKED"] = CHECKBOX_WBW_CHECKED,
	// app["SURAH_NUM"] = SURAH_NUM
	// app["FSIZE_AR"] = FSIZE_AR
	// app["FSIZE_LAT"] = FSIZE_LAT
	// L.app.set(APP_ID, app)
	L.fcanvas.close()
}
...
function func_opt() {
	...
	L.fcanvas.name(TXT["page"]["word"]["option"])
	L.fcanvas.add(rslider1)
	L.rslider.name(RSLIDER_AR, TXT["page"]["word"]["arabic_size"])
	...
	L.rslider.name(RSLIDER_LAT, TXT["page"]["word"]["latin_size"])
	...
	L.rbutton.onchange(rbutton1, function() {
		change_lang('id')
	})
	...
	L.rbutton.onchange(rbutton2, function() {
		change_lang('en')
	})
...
function func_theme() {
	...
	var arr = [
    {'title':'Day', 'create':L.rbutton.create(rbtn[0]), 'disabled':true, 'litegold':'0', 'litepoint':'0', 'url': FE_URL + './assets/img/theme/day.png', 'func' : 'L.theme.change("day")'},
    {'title':'Night', 'create':L.rbutton.create(rbtn[1]), 'disabled':false, 'litegold':'1', 'litepoint':'100', 'url': FE_URL + './assets/img/theme/night.png', 'func' : 'L.theme.change("night")'},
    {'title':'Sunset', 'create':L.rbutton.create(rbtn[2]), 'disabled':false, 'litegold':'1', 'litepoint':'100', 'url': FE_URL + './assets/img/theme/sunset.png', 'func' : 'L.theme.change("sunset")'},
    {'title':'Earth', 'create':L.rbutton.create(rbtn[3]), 'disabled':false, 'litegold':'1', 'litepoint':'100', 'url': FE_URL + './assets/img/theme/earth.png', 'func' : 'L.theme.change("earth")'},
    {'title':'Sky', 'create':L.rbutton.create(rbtn[4]), 'disabled':false, 'litegold':'1', 'litepoint':'100', 'url': FE_URL + './assets/img/theme/sky.png', 'func' : 'L.theme.change("sky")'},
    {'title':'Galaxy', 'create':L.rbutton.create(rbtn[5]), 'disabled':false, 'litegold':'1', 'litepoint':'100', 'url': FE_URL + './assets/img/theme/galaxy.png', 'func' : 'L.theme.change("galaxy")'},
    {'title':'Forest', 'create':L.rbutton.create(rbtn[6]), 'disabled':false, 'litegold':'1', 'litepoint':'100', 'url': FE_URL + './assets/img/theme/forest.png', 'func' : 'L.theme.change("forest")'}
	]
	...
	L.fcanvas.add(div)
	L.fcanvas.name(TXT["page"]["word"]["theme"])
...
function sidebar_set () {
	...
	var logo_click = function() {
		// L.page.open('page_home', page_func_order[0]())
		L.page.open('page_home', page_func_order[PAGE_HOME_ORDER]())
	}
	L.sidebar.img_click(logo_click)

	var sidebar_img = FE_URL + "assets/img/enlite/enlite.svg"
...
function header_set() {
	L.header.set()
	L.header.menu.set()
	var menu_logo = FE_URL + "assets/lite.img/sidebar.svg"
	L.header.menu.img(menu_logo)
	L.header.logo.set()
	var on_submit = function (value) {console.log(value)}
	L.header.searchbar.set(on_submit)
	...
	var header_img = FE_URL + "assets/img/enlite/enlite.svg"
