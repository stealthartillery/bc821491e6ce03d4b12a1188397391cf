   ...
   //  	const remote = require('electron').remote
	...
	function header() {
		...
		function set() {
			...
			$(div).css({'height': '-webkit-fill-available'})
			div.appendChild(tr)
				...
				if(typeof(EventSource) !== "undefined") {
				  ...
				  source.onmessage = function(event) {
					  notif = JSON.parse(event.data);
						var bell = str_to_elm(I.obj['bell'])
						$(bell).css({'margin-right':'10px', 'width': '20px', 'height': '20px'})
						$(bell).addClass('goldenrod')

				  	if (notif !=0) {
				  		document.querySelector('#acc_notif').innerHTML = notif
				  		$('#acc_notif').removeClass('transparent')
				  		document.querySelector('.title.notif').innerHTML = bell.outerHTML+'<span style="vertical-align: -webkit-baseline-middle;">'+TXT['header']['word']['notification']+' ('+notif+')'+'</span>'
				  	}
				  	else {
				  		document.querySelector('#acc_notif').innerHTML = ''
				  		$('#acc_notif').addClass('transparent')
				  		document.querySelector('.title.notif').innerHTML = bell.outerHTML+'<span style="vertical-align: -webkit-baseline-middle;">'+TXT['header']['word']['notification']+'</span>'
				  	}
			...
			function set(obj) {
			  ...
			  document.querySelectorAll('.canvas .header table tr')[0].appendChild(td)
				...
				function account_create(obj) {
					...
					var account = str_to_elm(I.obj['account'])
					$(account).css({'margin-right':'10px', 'width': '20px', 'height': '20px'})
					$(account).addClass('goldenrod')
					var bell = str_to_elm(I.obj['bell'])
					$(bell).css({'margin-right':'10px', 'width': '20px', 'height': '20px'})
					$(bell).addClass('goldenrod')
					var info = str_to_elm(I.obj['info'])
					$(info).css({'margin-right':'10px', 'width': '20px', 'height': '20px'})
					$(info).addClass('goldenrod')
					var exit = str_to_elm(I.obj['exit'])
					$(exit).css({'margin-right':'10px', 'width': '20px', 'height': '20px'})
					$(exit).addClass('goldenrod')
					...
					div_notif.innerHTML = bell.outerHTML+'<span style="vertical-align: -webkit-baseline-middle;">'+TXT['header']['word']['notification']+'</span>'
					...
					div_profile.innerHTML = account.outerHTML+'<span style="vertical-align: -webkit-baseline-middle;">'+TXT['header']['word']['profile']+'</span>'
					...
					div_info.innerHTML = info.outerHTML+'<span style="vertical-align: -webkit-baseline-middle;">'+TXT['header']['word']['information']+'</span>'
					...
					div_logout.innerHTML = exit.outerHTML+'<span style="vertical-align: -webkit-baseline-middle;">'+TXT['header']['word']['sign_out']+'</span>'
					...
					for (var i=0; i<obj.length; i++) {
						...
						// var _div = str_to_elm(
						// 	'<div class="title" onclick="'+obj[i]['func']()+'">'+
						// 	'<div>'+
						// 	'</div>'+
						// 	'</div>'
						// )
						var _div = document.createElement('div')
						_div.setAttribute('class', 'title')
						_div.innerHTML = obj[i]['title']
						// var func_ = func_create(obj[i]['func'])
						_div.addEventListener('click', obj[i]['func'])
						div_ddc.appendChild(_div)
					}
    			...
    			var initial_name = str_to_elm('<div style="color: goldenrod; font-family: Libre Baskerville; font-size: 12px; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">' +
					...
					function func_notif() {
						...
						$(bell).css({'vertical-align':'top'})
						L.fcanvas.title(bell.outerHTML+'<span style="vertical-align: top;">'+TXT['header']['word']['notification']+'</span>')
					...
					function func_info() {
						...
						var accord = L.accordion.create('asd1')
							...
							if (APP_PLATFORM == 'des') {
								btn.addEventListener('click', function() {
									...
									shell.openExternal(HOST+'tradelite.lanterlite.com/store/lanterlite')
									// window.open(HOST+'tradelite.lanterlite.com/store/lanterlite', "Lanterlite", "resizable,scrollbars,status") 
								})
							}
							else
								btn.addEventListener('click', function() { window.open(HOST+'tradelite.lanterlite.com/store/lanterlite') } )

							function license_table_create(user_licenses) {
								var _license_table = str_to_elm(
									'<div>'+
									'</div>'
								)
								for (var i=0; i<user_licenses.length; i++) {
									var div = str_to_elm(
										'<img class="std" style="width: 100px; box-sizing: border-box; margin: 5px;" src="'+user_licenses[i]['license_img']+'">'
									)
									_license_table.appendChild(div)
								}
								return _license_table
							}

							var user_licenses = JSON.parse(atob(sessionStorage.getItem(MD5("user"))))['user_license']
							if (user_licenses == undefined)
								user_licenses = []

							var div = str_to_elm(
								'<div>'+'<div style="text-align: center; padding: 0px 0px 10px 0px;">'+
									((user_licenses.length != 0)? license_table_create(user_licenses).outerHTML : '('+TXT['header']['word']['no_license_card']+')')+
								'</div>'+'</div>'
							)
							div.appendChild(btn)
						...
						L.fcanvas.title(info.outerHTML+'<span style="vertical-align: top;">'+TXT['header']['word']['information']+'</span>')
					...
					function func_profile() {
						$(account).css({'vertical-align':'top'})
						L.fcanvas.title(account.outerHTML+'<span style="vertical-align: top;">'+TXT['header']['word']['profile']+'</span>')
				...
				function sign_menu_create() {
					...
					if (APP_PLATFORM == 'des') {
						inp_su.addEventListener('click', function() {
							const { shell } = require('electron')
							shell.openExternal(CORELITE_URL + 'register')
							// window.open(CORELITE_URL + 'register', "Lanterlite", "resizable,scrollbars,status")
						})
					}
					else
	...
	function checkbox() {
		...
		function create(id) {
  		...
  		var span2 = document.createElement('span')
  		span2.className = 'title theme_clr_text1'
  		span2.id = 'title'
...
/* obj_req = {'method':'get', 'url':'https...', 'callback':'function(obj){}'} */
  	...
  	div_date.setAttribute('style', "font-size: 16px; font-weight: 600; color: green; text-align: left; font-family: sans-serif;")
		...
		if (obj['link'] != '') {
			...
			for (var i=0; i<obj['link'].length; i++) {
				...
				var a = str_to_elm('<a target="_blank">'+obj['link'][i]['title']+'</a>')
				if (APP_TYPE == 'amp')
